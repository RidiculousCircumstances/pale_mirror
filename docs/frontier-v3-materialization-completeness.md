...
| MAT-003 | P0 | M1 foundation | Generic `STRUCTURAL_REPAIR` and `DECONTAMINATION` still directly consume one exact item and change target blocks in the live path. The new persisted `SettlementServiceWork` owns one exact qualified resident, two stations/topologies/cursors, source slot, input, target and endpoint intents; the closed `SERVICE_WORK` cause/codec and its own NeoForge registry owner reject actor substitution and retain worker-death recovery. Its loaded input executor now moves only the exact tagged depot stack to the exact HOT worker body at the immutable input station, with durable-before-effect confirmation and restart inspection; a nonempty hand is a conflict and is never overwritten. The planner does not yet emit work or a lease, so it remains unreachable in ordinary worlds and no work is visibly in progress. Wire planner admission, observed approach/progress, terminal endpoint and restart/native evidence; retain existing effect executors only as terminal atomic adapters. |
