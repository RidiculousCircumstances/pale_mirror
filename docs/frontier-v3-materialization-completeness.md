...
| MAT-003 | P0 | M1 active | The canonical decontamination scan now atomically activates its strategic task and retains one exact `SettlementServiceWork`: qualified medic, intact infirmary, active exact depot slot, two immutable stations/topologies/cursors, input-issue intent and endpoint intent. The sealed `SERVICE_WORK` cause/codec and derived assignment reject actor substitution; forged admissions are rejected by recomputing the exact candidate. Its loaded input executor moves only the exact tagged depot stack to the exact HOT worker body at the immutable input station, with durable-before-effect confirmation and restart inspection; a nonempty hand is a conflict and is never overwritten. No scene yet advances the retained approach/progress, so admitted work remains `PREPARED` and endpoint execution is correctly unreachable. `STRUCTURAL_REPAIR` remains on its prior direct path until its own service form is admitted. Add observed approach/progress, terminal endpoint, recovery and native evidence; retain existing effect executors only as terminal atomic adapters. |
