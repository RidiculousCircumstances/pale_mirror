...
- the harvest, hive-growth and hive-nutrient pilot scenarios assert terminal
  diagnostics/receipts and result frames, not the missing in-progress actors or
  physical frontiers.

| ID | Priority | Current strongest evidence | Gap and closure requirement |
|---|---:|---|---|
| MAT-001 | P0 | M1 | Resource-site preparation writes a whole field, stage projection changes every crop together, and harvest resets the whole field plus writes 64 wheat to the depot. The harvest job names a worker, but no worker lease, field route, action or carried output participates. Fresh-world field projection may be explicitly classified as class-A baseline; every later preparation/repair remains class C. Add retained field-work progress and exact worker HOT harvest; make continuing growth spatially progressive under class E. |
