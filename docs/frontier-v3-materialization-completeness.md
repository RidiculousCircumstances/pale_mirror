...
| MAT-002 | P0 | M2 partial; M3 not accepted | Fresh-world schema 117/envelope 31 retain staged work and one bounded immutable worker-to-workshop topology/cursor through a semantic port with distinct input/work stations, headroom and public-circulation join. Typed reducers accept only a matching HOT lease plus the observed exact worker at the retained station; no executor mutates a job directly. The registered class-C `PRODUCTION_WORK` executor materializes only that worker, advances observed approach/input/processing, and defers exact chest replacement until `OUTPUT_READY`. A blocked job remains attached through durable release, then `ProductionWorkSceneFinalized` removes it and releases its exact reservation without a zombie lease. Focused early-intent, forged-observation and finalized-after-close tests pass; Scene 44/44 and Economy 17/17 pass. Native `disposable_materialized_production_work_restart` run `75f45107-732a-43dc-996b-1f47a9e0244b` found the exact `INDUSTRIAL WORKER`, reached terminal `FULFILLED`, then gracefully restarted with the exact bread receipt intact. It is M2 partial rather than closed: add a dedicated GameTest for approach/input/processing plus death/theft/broken/blocked/restart paths. The current native frame is technically valid but its narrow graybox corridor obscures the worker, so a readable player-height worker/station presentation is still M3 debt. |
