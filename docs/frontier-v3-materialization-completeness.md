...
| MAT-003 | P0 | M1 foundation | Generic `STRUCTURAL_REPAIR` and `DECONTAMINATION` still directly consume one exact item and change target blocks in the live path. The new persisted `SettlementServiceWork` owns one exact qualified resident, station, topology/cursor, input, target and intent; the closed `SERVICE_WORK` cause/codec and its own NeoForge registry owner reject actor substitution and retain worker-death recovery. It is not yet emitted or HOT-executed, so no work is visibly in progress. Wire the planner, exact item hand-off, observed approach/progress, terminal endpoint and restart/native evidence; retain existing effect executors only as terminal atomic adapters. |
