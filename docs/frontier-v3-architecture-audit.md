...
| V3-AUD-037 | P0 | Confirmed defect | Resource-site preparation/harvest, production transformation, generic structural repair and decontamination have strict exact-item/block endpoint executors, but no scene or other retained physical actuator makes their named worker/team/machine execute the duration-bearing job. Harvest's `workerId` is used only in its receipt; all 64 crops and the depot output change in one executor call. |
