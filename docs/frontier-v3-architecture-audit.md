...
| V3-AUD-019 | IN PROGRESS | T0.1/T0.2 now carry `BodyPosition` through every current scene family: ambient-to-scene captures, scene release, and HOT death evidence cannot use a support cell. Fresh schema-100 and persistence-envelope v9 reject all previous bytes before hydration/WAL replay. T0.3 now pins one bounded immutable `TerrainSurfacePlan` (baseline plus sparse surveyed support columns) in the bootstrap identity and snapshot header; it is never a loaded-world height query. Raised route surfaces compile their own `ROUTE_FOUNDATION` cells from that survey, and observed foundation loss blocks the same retained edge instead of allowing a hidden bypass or a cosmetic repair. Focused sparse-survey/recovery coverage proves that one sampled column neither spreads into a height map nor disappears on hydration; a native stepped-route scenario proves provider-owned footing, materialization and graceful restart without player scaffolding. `OperationTravel` and every `OperationAssembly` member retain exact topology/cursors, and the registered provider proves the support-to-feet boundary. T0.5 has native `COMPILED`, scoped `SETTLED` and scoped graceful-`RELOADED` Foundry evidence: fresh unclaimed expected air is explicit pending projection work, while exact plan/ledger/block provenance remains required for current materialization and all other drift stays an error. Runtime Foundry reports one semantic port as `OPEN`/`BLOCKED`/`UNVERIFIED` and each retained edge as `OPEN`/`PENDING`/`BLOCKED`/`UNVERIFIED`; the native player block/restart proof keeps the same Infirmary port `BLOCKED` after recovery without an inferred alternative entrance. The debt remains open for facility sites on distinct surveyed datums, bridge/rail-grade evidence, an exact foundation repair/replan lifecycle and cross-family HOT/COLD continuity. |
...
| V3-AUD-022 | IN PROGRESS | The schema-99 foundation now retains a distinct bounded `RouteMaintenance` aggregate for one PM-owned semantic surface/foundation loss, its exact local engineering team, optional exact cargo and shared COLD/HOT assembly. It is not a structural-repair or bypass alias; generic repair remains forbidden for `route:frontier-network`. The registered process now makes one confirmed source decrement create the cargo identity, advances that same team/cargo, and lets only an observed target repair remove its exact delta. A HOT/COLD regression also closed the admission race: a retained PM baseline loss blocks bypass admission at the loss itself, not only after maintenance happens to be retained, so queue order cannot turn repair into a hidden reroute. Native restart evidence found one further confirmed ownership gap: a completed engineering assembly was absent from the ambient reservation index, so an ordinary return visit could reopen the exact COLD-ready engineer as ambient `WORK` before its engineering scene prepared. The shared `FrontierSceneAdmission` index now reserves precisely its deterministic next engineering candidate; the ambient executor still discards an exact CLOSED ambient body before honoring that successor reservation. The repeat run proved that HOT hand-off, then found two exact continuation defects: `ROUTE_MAINTENANCE` correctly names `route:frontier-network` as its semantic cause but its NeoForge executor incorrectly treated that cause as the retained maintenance operation, silently deferring every valid prepared work intent; and aggregate validation attempted to validate a retained terminal/conflict assembly as a new active work front, quarantining the runtime. `RouteMaintenanceStateSupport.workOperation` now resolves only the exact bound operation subject, while `EngineeringWorksite.validate` skips historical non-BUILDING assemblies; focused normal and terminal/conflict regressions prove both repairs. Native evidence now covers both durable split points without a force-load: `19d96775-65e4-4d4c-874f-45a3dac1e6fb` restarts after source pickup and before target repair, while `d8e7ef3d-196a-4cde-94c1-31a541dabf42` restarts after the observed target repair while the same tool-holding engineer is on `RETURN_DEPOT`; it reaches the exact `human_equipment_returned` receipt and terminal compaction with the original edge available. Native proof also exposed a real terrain seam: a drained engineer stood on surveyed support Y=63 while the retained route deck was Y=64, and the old constant-Y corridor silently deferred forever. The approach compiler now derives its bounded candidate supports from immutable `TerrainSurfacePlan`, retains only adjacent one-block grades, and exposes the exact rejection reason read-only; focused regressions accept the one-step case and reject a cliff. This is an incremental terrain-provider correction, not closure of V3-AUD-019's full port/topology gate. Explicit development fixtures may retain an already-admitted detour solely to exercise its own physical executor; they are not a production cause. A completed repair may reopen only retained edges no longer affected by another loss; active interrupted operations receive no invented resume. Remaining exit evidence is the declared focused foreign/unavailable source/target and exact running-effect recovery matrix, followed by the critical gate. |
...
| V3-AUD-031 | IN PROGRESS | Route-maintenance source and target selection now consumes the existing bounded `INVALID`/`DEFERRED`/`RUNNABLE` scheduler: an unloaded source or target defers only its own exact intent, while malformed canonical origin/target remains selected and becomes visible `UNKNOWN_AFTER_RESTART` evidence instead of silently blocking the family. Focused scheduling tests, the 39/39 HOT/COLD scene slice and the complete critical gate pass (271/271 GameTests, build/package). Native run `5308ec46-e3c8-40ff-b60e-afe77e2e811c` preserves the ordinary repair/return/restart path. Exit evidence remains a two-maintenance native scenario with one naturally COLD source and one loaded repair. |
| V3-AUD-032 | IN PROGRESS | Replace global serialized admission/progress with a bounded deterministic round-robin over retained maintenance owners plus one separately checked new candidate per scan. Distinct cells may proceed only with disjoint retained teams and cargo identities; an exact source stack already bound by a nonterminal material-loading intent is unavailable until its observed transition releases or changes it. Preserve `MAX_MAINTENANCE=12`, one aggregate per cell and no forced chunk load. Exit evidence: pure multi-loss/order/cargo-reservation/recovery tests, materialized negative ownership tests, and the V3-AUD-031 native scenario with one COLD source plus another loaded repair. |
