...
| V3-AUD-011 | OPEN — H0.3 | The composition root is in `runtime` and model has a zero-tolerance source guard against imports from `runtime`, `process` or `persistence`. All 23 snapshot/WAL codecs now reside in `persistence`; its only model boundary is public immutable value/state data, and the debt ratchet permits zero codec classes in `model`. The full critical gate passes (248/248 GameTests, build and packaged-JAR verification). The remaining debt is the 33 behavior/process classes and their planners/reducer, which must move as true process modules rather than as model adapters. |
