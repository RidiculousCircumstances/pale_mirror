...
| V3-AUD-015 | IN PROGRESS | Recovery must inspect only the verified snapshot header before engine creation, select the exact installed persisted ruleset, then prove world ID/seed and pin that bootstrap. It must preserve fail-closed handling for foreign headers and unavailable selectors; a real r41 restart is required before closure. |
| V3-AUD-016 | IN PROGRESS | Versioned strategic hydration must recognise only the proven pre-assault r41 ordinal layout when its field-target evidence makes that layout unambiguous, map objective/task kinds explicitly, and retain normal schema-78/79 assault layout otherwise. A pure old-byte regression plus a real r41 restart/replay are required before closure. |
