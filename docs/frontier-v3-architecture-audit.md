...
| V3-AUD-024 | CLOSED | The bounded volatile bridge now retains an exact joining body until the global UUID index resolves to that same Java object or the body is removed. It never treats `isAddedToLevel()` or elapsed time as identity publication; pre-index materialization and restart-absence inference defer visibly as `PENDING_UNINDEXED`. While that first body is pending, a later otherwise-managed body with the same UUID is rejected at the cancellable entity-join boundary before Minecraft can expose a flickering duplicate; the first candidate remains sole bridge owner. The focused negative/recovery GameTest proves both the rejected second admission and the retained first candidate, then releases only when that candidate is discarded. |
