...
| V3-AUD-038 | P0 | Confirmed defect; replacement contract accepted | Exact `RoutePatrol` members advance through COLD waypoint updates, including direct actor-body rewrites, while resident ambient goals reduce security work to generic settlement `GUARD`; hive mobilisation has evidenced assembly/departure and battle endpoints but no same-roster HOT/COLD march proof between them. Generic local motion is therefore able to coexist with invisible assigned movement. `docs/frontier-v3-route-patrol-contract.md` defines the fresh-world-only route-patrol replacement; no compatibility decoder or bridge is permitted. |
...
| V3-AUD-037 | OPEN | `MAT-001` and `MAT-002` are closed at M2. `MAT-002` now retains the exact worker, stations, stage and cursor across a filesystem restart, turns a loaded player block at a retained future body into one typed `ROUTE_BLOCKED` fact, then drains/finalizes only that job and reservation; no detour, replacement worker or inferred receipt is permitted. Its native real-player scenario and dedicated scene/filesystem-recovery tests pass. A reusable M3 presentation grammar remains open. Close `MAT-003` by introducing the same reusable actor/machine work lifecycle for repair and decontamination, retaining the exact worker/actuator, semantic station, inputs, progress and intervention path through HOT/COLD/restart. Endpoint executors remain terminal effect adapters. Every flow needs ordinary, worker/input/station loss and restart evidence plus an in-progress player-height frame. |
| V3-AUD-038 | OPEN | Close `MAT-004` and `MAT-005`: exact patrol and expedition owners must retain one terrain-aware route/cursor/roster through departure, HOT movement, contact and retreat. Ambient `WORK`/`GUARD`/`PATROL` cannot satisfy an assigned operation. Prove observed same-cursor motion, obstruction, member/controller loss and restart. |
| V3-AUD-039 | OPEN | Close `MAT-006`, `MAT-007` and `MAT-010`: add exact-recipient lifecycle stages for provision/birth, interruptible hiveroot transfer/digestion/morphogenesis for hive growth, and derived HOT cues plus exact-contact evidence for health/nutrition. Existing exact consumption/endpoint receipts and medical scene stay valid foundations but cannot close this finding without continuous HOT evidence. |
...
| V3-AUD-042 | IN PROGRESS | `ProductionProcess.planCompletion` now refuses to prepare a materialized terminal transform while the exact production lease is not `CLOSED`; `ProductionWorkBehavior.releasePlan` instead schedules exactly one same-job completion after `SceneLeaseReleased`. Commit `faec9ec5` has focused HOT-deferral/closed-lease/accepted-receipt coverage, Economy 17/17, Scene 56/56 and the 287-GameTest critical gate. Fresh `frontier-v3-live-r70` starts the same SHA-pinned artifact with no quarantine. Required before closure: a native production run that exercises this release-before-confirm order without quarantine. |
