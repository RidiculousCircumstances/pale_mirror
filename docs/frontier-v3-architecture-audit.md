...
| V3-AUD-003 | OPEN — H0.3 | The registry now rejects unowned command/schedule/event input before dispatch, but `FrontierWorldRuntimeDefinition` still contains the domain planner/reducer implementations. Split those into the registered process modules before closing. |
