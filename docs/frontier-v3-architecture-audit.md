...
| V3-AUD-003 | OPEN — H0.3 | Scheduled routing is now a closed process-catalog map and its runtime string-switch ceiling is 0. The registry rejects unowned command/schedule/event input before dispatch, but `FrontierWorldRuntimeDefinition` still contains 34 command type tests and 104 reducer cases; the new ratchets prevent either dispatcher from growing while they are split into registered process modules. |
...
| V3-AUD-011 | OPEN — H0.3/H0.4 | The explicit catalog is a composition foundation only; move behavior modules first, then codecs/persistence without reverse dependencies. |
| V3-AUD-012 | OPEN — H0.3 | Descriptor/catalog startup now rejects duplicate ownership, missing stable codecs and unregistered command/schedule/event input. Finish precise per-process emitted-payload contracts and representative codec round trips before closing. |
