...
| V3-AUD-034 | CLOSED | The release gate treats an absent provenance claim as `PENDING` projection work and opens only a physically exact claimed `HIVE_COCOON`; an exact prior claim plus a changed block remains a visible conflict. The reusable pure classification regression and exact baseline-palette check pass. Native `disposable-hive-mobilization-release-restart` run `30d23def-8dfa-474a-a6a2-1e4cc366aecd` proves naturally demanded `WAKING → ASSEMBLING → HOT` for the exact three-member group and preserves it across graceful restart; clean frames show the readable HIBERNACULUM assembly board and the same three Zombies. The complete critical gate passes: `guardrails`, `check`, build/package verification and 278/278 GameTests. During that gate an unrelated repair-admission cost defect was also corrected: approach compilation now indexes the immutable route surface once and rejects an overlong candidate before expensive column resolution, rather than rescanning the entire network for every candidate cell. The former hanging exact-crew regression completes in 9 seconds. |
