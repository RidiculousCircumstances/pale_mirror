...
| V3-AUD-022 | IN PROGRESS | Route maintenance needs its own bounded exact owner: a confirmed source decrement creates one cargo identity, the retained engineering team advances that cargo through the same COLD/HOT work-site protocol, and only an observed target repair removes its exact delta. It may reopen only the affected retained edges for which no other semantic surface/foundation loss remains; active interrupted operations do not receive an invented resume. A generic repair intent remains forbidden for `route:frontier-network`. Exit evidence is normal loaded repair, unavailable/foreign source or target, restart between pickup and target postcondition, and a native player break → exact repair → same-edge recovery scenario without force-loading. |
...
| V3-AUD-024 | CLOSED | The bounded volatile bridge now retains an exact joining body until the global UUID index resolves to that same Java object or the body is removed. It never treats `isAddedToLevel()` or elapsed time as identity publication; pre-index materialization and restart-absence inference defer visibly as `PENDING_UNINDEXED`. The focused negative/recovery GameTest proves one PREPARED resident stays protected from duplicate admission and releases only after its candidate is discarded. |
...
| V3-AUD-027 | IN PROGRESS | The correction must expose one owning-thread immutable canonical state/revision/instant view for NeoForge physical adapters while retaining `CheckpointImage` exclusively as an explicit persistence/diagnostic boundary. Every physical command must take its expected revision/instant from that view, mutations must still enter only through `submit`, and tests must prove that repeated physical reads plus a command do not call the state codec until an explicit checkpoint. A fresh same-route r56 JFR must then compare the removed codec samples and entry tail latency without claiming that a teleport-driven first chunk load is simulation work. |
