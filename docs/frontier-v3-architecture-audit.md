...
| V3-AUD-042 | IN PROGRESS | `ProductionProcess.planCompletion` now refuses to prepare a materialized terminal transform while the exact production lease is not `CLOSED`; `ProductionWorkBehavior.releasePlan` instead schedules exactly one same-job completion after `SceneLeaseReleased`. The focused regression proves HOT deferral, closed-lease scheduling and accepted terminal receipt. Required before closure: focused Economy/Scene GameTests and a fresh native production run with no runtime quarantine. |
