...
| V3-AUD-003 | OPEN — H0.3 | Scheduled routing is a closed process-catalog map and `FrontierWorldRuntimeDefinition` is a 62-line `runtime` composition root with no command/reducer branches. All behavioral processes, the trusted-command planner and the event reducer now reside in `process`; `model` has no reverse package dependency. The remaining 34 trusted-command branches and 104 reducer cases must become independently registered closed process handlers with exact payload contracts rather than two bounded dispatchers. The architecture ratchet and full critical gate pass (248/248 GameTests, build and packaged-JAR verification). |
