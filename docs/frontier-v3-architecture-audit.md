...
| V3-AUD-037 | P0 | IN PROGRESS | Resource-site preparation/harvest, production transformation, generic structural repair and decontamination have strict exact-item/block endpoint executors, but initially no retained physical actuator made their named worker/team/machine execute duration-bearing work. Harvest's schema-114 named-worker topology/cursor and one-cell HOT frontier now have normal, negative, restart and native visible evidence: `disposable_redwillow_harvest` run `5110c424-f5b0-4321-894e-a6fdec3b7f36` restarts during the same active harvest and reaches its exact terminal receipt. Production, repair and decontamination remain actorless, so this cross-class P0 finding remains open. |
| V3-AUD-038 | P0 | Confirmed defect | Exact `RoutePatrol` members advance through COLD waypoint updates, but resident ambient goals reduce security work to generic settlement `GUARD`; hive mobilisation has evidenced assembly/departure and battle endpoints but no same-roster HOT/COLD march proof between them. Generic local motion is therefore able to coexist with invisible assigned movement. |
...
| V3-AUD-037 | OPEN | Close `MAT-001` through `MAT-003`: introduce a reusable actor/machine work lifecycle and retain the exact worker/actuator, semantic station, inputs, progress and intervention path through HOT/COLD/restart. `MAT-001`'s first per-crop executor must additionally replace its direct local `moveTo` approach with one retained bounded 3D pedestrian topology from the worker's current canonical surface to a semantic field/work-cell port; neither a flat offset nor Minecraft navigation may become an unrecorded second route authority. The current endpoint executors remain terminal effect adapters. Each flow needs ordinary, worker/input/station loss and restart evidence plus an in-progress player-height frame. |
