...
const SCHEMA = 1;
const EVIDENCE_ACTIONS = new Set(['walk', 'look', 'break', 'open_container', 'withdraw', 'deposit', 'die', 'wait']);
...
export function validateScenario(scenario) {
  ...
  for (const [phase, allowed] of [['setup', SETUP_ACTIONS], ['actions', EVIDENCE_ACTIONS]]) {
    ...
    for (const action of actions) {
      ...
      if (['walk', 'look', 'break', 'open_container'].includes(action.type)) validatePosition(action.position ?? action.at);
