...
final class MarketPayloadCodecs {
    ...
    static PayloadCodec workOrderCancelled() { return new PayloadCodec() {
        ...
        @Override public byte[] encode(FrontierPayload payload) { return write(output -> {
            ...
            subject(output, cancelled.orderId()); subject(output, cancelled.jobId()); output.writeByte(cancelled.reason().ordinal());
        }); }
        @Override public FrontierPayload decode(byte[] bytes) { return read(bytes, input -> {
            ...
            return new MarketWorkOrderCancelled(order, job, ProductionBlockReason.values()[reason]);
        }); }
    }; }
    ...
    static PayloadCodec cancelled() { return new PayloadCodec() {
        ...
        @Override public byte[] encode(FrontierPayload payload) { return write(output -> {
            ...
            subject(output, cancelled.demandId()); output.writeByte(cancelled.reason().ordinal());
        }); }
        @Override public FrontierPayload decode(byte[] bytes) { return read(bytes, input -> {
            ...
            return new MarketDemandCancelled(demand, MarketDemandCancellationReason.values()[reason]);
        }); }
    }; }
    ...
    private static void writeDemand(DataOutputStream output, MarketDemand demand) throws IOException {
        ...
        output.writeLong(demand.expiresAtTick()); output.writeByte(demand.status().ordinal());
    }
    private static MarketDemand readDemand(DataInputStream input) throws IOException {
        ...
        return new MarketDemand(id, buyer, reason, item, count, maximum, opened, expiry, MarketDemandStatus.values()[status]);
    }
    ...
    private static void writeOrder(DataOutputStream output, MarketWorkOrder order) throws IOException {
        ...
        subject(output, order.taskId()); subject(output, order.jobId()); subject(output, order.reservationId()); output.writeLong(order.acceptedTotalPrice().raw()); output.writeByte(order.status().ordinal());
    }
    private static MarketWorkOrder readOrder(DataInputStream input) throws IOException {
        ...
        return new MarketWorkOrder(id, demand, quote, seller, task, job, reservation, total, MarketWorkOrderStatus.values()[status]);
    }
