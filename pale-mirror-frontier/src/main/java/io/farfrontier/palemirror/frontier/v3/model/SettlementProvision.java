...
public record SettlementProvision(SubjectId settlementId, int cycleOrdinal, long startedAtTick, int requiredRations,
                                  int fulfilledRations, List<SettlementRationAllocation> allocations, int nextAllocation,
    ...
    static SettlementProvision idle(SubjectId settlementId) {
        return new SettlementProvision(settlementId, 0, 0L, 0, 0, List.of(), 0, SettlementProvisionStatus.IDLE, Optional.empty());
    }

    static SettlementProvision started(SubjectId settlementId, int cycleOrdinal, long tick, int required,
                                       List<SettlementRationAllocation> allocations) {
        if (allocations.isEmpty()) {
            return new SettlementProvision(settlementId, cycleOrdinal, tick, required, 0, List.of(), 0,
                    SettlementProvisionStatus.SHORTAGE, Optional.empty());
        }
        return new SettlementProvision(settlementId, cycleOrdinal, tick, required, 0, allocations, 0,
    ...
    SettlementProvision beginPhysical(PhysicalIntentId intentId) {
        ...
        return new SettlementProvision(settlementId, cycleOrdinal, startedAtTick, requiredRations, fulfilledRations, allocations,
    ...
    SettlementProvision consumeCurrent(SubjectId itemId, int count) {
        ...
        if (next < allocations.size()) return new SettlementProvision(settlementId, cycleOrdinal, startedAtTick, requiredRations, fulfilled,
                allocations, next, SettlementProvisionStatus.IN_PROGRESS, Optional.empty());
        return new SettlementProvision(settlementId, cycleOrdinal, startedAtTick, requiredRations, fulfilled, List.of(), 0,
    ...
    SettlementProvision shortage() {
        ...
        return new SettlementProvision(settlementId, cycleOrdinal, startedAtTick, requiredRations, fulfilledRations, List.of(), 0,
