    ...
    static PayloadCodec residentTransition() {
        return new PayloadCodec() {
            ...
            @Override public byte[] encode(FrontierPayload payload) { return FrontierWorldPayloadCodecs.encodeProduction(output -> {
                ...
                FrontierWorldPayloadCodecs.writeSubject(output, transition.residentId()); output.writeByte(transition.status().wireTag()); output.writeLong(transition.atTick());
            }); }
            @Override public FrontierPayload decode(byte[] bytes) { return FrontierWorldPayloadCodecs.decodeProduction(bytes, input -> {
                ...
                return new ResidentHealthTransition(resident, FrontierWireTags.require(ResidentHealthStatus.class, status), input.readLong());
            }); }
        };
    }
    ...
    static PayloadCodec quarantineTransition() {
        return new PayloadCodec() {
            ...
            @Override public byte[] encode(FrontierPayload payload) { return FrontierWorldPayloadCodecs.encodeProduction(output -> {
                ...
                FrontierWorldPayloadCodecs.writeSubject(output, transition.settlementId()); output.writeByte(transition.status().wireTag()); output.writeLong(transition.atTick());
            }); }
            @Override public FrontierPayload decode(byte[] bytes) { return FrontierWorldPayloadCodecs.decodeProduction(bytes, input -> {
                ...
                return new SettlementQuarantineTransition(settlement, FrontierWireTags.require(SettlementQuarantineStatus.class, status), input.readLong());
            }); }
        };
    }
}
