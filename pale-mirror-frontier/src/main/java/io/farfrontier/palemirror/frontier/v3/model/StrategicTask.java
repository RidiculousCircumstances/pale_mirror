...
record StrategicTask(SubjectId id, SubjectId objectiveId, SubjectId ownerId, StrategicTaskKind kind,
    ...
    StrategicTask {
        ...
        if (kind != StrategicTaskKind.GROW_HIVE_ORGANISM && kind != StrategicTaskKind.PRODUCE_BREAD && kind != StrategicTaskKind.DELIVER_BREAD_TO_HIVE && infectionTarget.isEmpty()) {
            throw new IllegalArgumentException("infection strategic task requires an infection target");
        }
        if ((kind == StrategicTaskKind.GROW_HIVE_ORGANISM || kind == StrategicTaskKind.PRODUCE_BREAD || kind == StrategicTaskKind.DELIVER_BREAD_TO_HIVE) && infectionTarget.isPresent()) {
