...
public record RecoveryImage(WorldId worldId, Optional<CheckpointImage> checkpoint, List<TransactionRecord> walTail) {
    public RecoveryImage {
        ...
        checkpoint.ifPresent(image -> {
            if (!worldId.equals(image.worldId())) throw new IllegalArgumentException("checkpoint world does not match recovery image");
        });
