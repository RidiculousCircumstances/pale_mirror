...
final class FrontierSceneLeaseValidationSupport {
    ...
    static void validate(FrontierBootstrap bootstrap, HumanPopulation population, Map<SubjectId, ActorLocation> actors, Map<SubjectId, StructureCondition> structures,
                         Map<SubjectId, RouteOperation> operations, Map<SubjectId, RouteConstruction> constructions, StrategicPlanState plans, Map<SceneLeaseId, SceneLease> leases,
        ...
        for (Map.Entry<SceneLeaseId, SceneLease> entry : leases.entrySet()) {
            ...
            Set<SubjectId> expected = FrontierSceneBehaviors.expectedMembers(bootstrap, population, actors, structures, operations, constructions, plans, lease, leasedOperations);
