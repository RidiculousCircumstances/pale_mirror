...
final class SettlementProvisionPayloadCodecs {
    ...
    private static SettlementProvision read(DataInputStream input) throws IOException {
        ...
        int required = FrontierWorldStateCodec.readCount(input); int fulfilled = FrontierWorldStateCodec.readCount(input); List<SettlementRationAllocation> allocations = new ArrayList<>();
        for (int index = 0, count = FrontierWorldStateCodec.readCount(input); index < count; index++) {
            allocations.add(new SettlementRationAllocation(FrontierWorldPayloadCodecs.readSubject(input).value(), FrontierWorldStateCodec.readCount(input)));
        }
        ...
        return new SettlementProvision(settlement, cycle, startedAt, required, fulfilled, List.copyOf(allocations), next, SettlementProvisionStatus.values()[status], intent);
    }
}
