...
class RouteSceneReturnRepairTest {
    ...
    void releasedFailedHotShipmentStillTriggersPatrolAndRouteConstruction() {
        ...
        SceneLease lease = lease(world, engine.checkpoint(), operation);
        ...
        submit(engine, world, new SceneLeaseReleased(lease.id(), memberPositions(operation)));
    ...
    private static SceneLease lease(WorldId world, io.farfrontier.palemirror.frontier.v3.api.CheckpointImage checkpoint, RouteOperation operation) {
        return new SceneLease(new SceneLeaseId("lease:route-scene-return-repair"), world, operation.id(), operation.cargoId(), operation.route().getFirst(),
                checkpoint.instant(), checkpoint.revision().value(), SceneLeaseStatus.PREPARED, sceneMembers(world, operation));
    }

    private static List<SceneMember> sceneMembers(WorldId world, RouteOperation operation) {
        return operation.participantIds().stream().map(actor -> new SceneMember(actor, SceneLease.deterministicEntityId(world, actor))).toList();
    }

    private static List<SceneMemberPosition> memberPositions(RouteOperation operation) {
        return operation.participantIds().stream().map(actor -> new SceneMemberPosition(actor, operation.route().getFirst())).toList();
    }
