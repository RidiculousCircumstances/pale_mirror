...
public final class ScenarioRuntime {
    public List<DomainEvent> accept(WorldState state, String scenarioId) {
        ...
        if (facility.status() == FacilityStatus.RECOVERING || facility.status() == FacilityStatus.OPERATIONAL) {
            ...
            DomainEvent resolved = new DomainEngine().event(state, DomainEventType.SCENARIO_RESOLVED, scenario.target(), scenario.sourceEventId());
            state.addEvent(resolved);
            return List.of(resolved);
        }
        scenario.setStatus(ScenarioStatus.INVESTIGATE);
        DomainEvent accepted = new DomainEngine().event(state, DomainEventType.SCENARIO_ACCEPTED, scenario.target(), scenario.sourceEventId());
    ...
    public List<DomainEvent> playerEntered(WorldState state, StoryAudienceId audience, WorldObjectId facilityId) {
        return state.scenarios().stream()
                ...
                .map(scenario -> {
                    ...
                    DomainEvent event = new DomainEngine().event(state, DomainEventType.FACILITY_INVESTIGATED, facilityId, scenario.sourceEventId());
    ...
    public List<DomainEvent> reconcileRecovery(WorldState state, WorldObjectId facilityId) {
        return state.scenarios().stream()
                ...
                .map(scenario -> {
                    scenario.setStatus(ScenarioStatus.RESOLVED);
                    DomainEvent event = new DomainEngine().event(state, DomainEventType.SCENARIO_RESOLVED, facilityId, scenario.sourceEventId());
                    state.addEvent(event);
                    return event;
                }).toList();
    }
