<# Run only a fixed primary-anchored Collector Hunyuan2mv diagnostic proposal. #>

[CmdletBinding()]
param(
    [ValidateSet('Prepare', 'Validate', 'Inference')]
    [string]$Phase = 'Validate',
    [ValidateSet('hunyuan2mv_v02_primary_anchored', 'hunyuan2mv_v03_primary_front', 'hunyuan2mv_v04_calibrated_secondary', 'hunyuan2mv_v05_canonical_turntable')]
    [string]$Trial = 'hunyuan2mv_v03_primary_front',
...
$candidateDirectory = "$workspace\candidates\biomass_collector\$Trial"
...
if ($Phase -eq 'Prepare') {
    ...
    & $python $prepare --trial $Trial --primary $primary --generated-root $generatedRoot --trace $trace --output $inputDirectory
    if ($LASTEXITCODE -ne 0) { throw 'Could not prepare the selected primary-anchored Hunyuan2mv inputs.' }
    exit 0
}
