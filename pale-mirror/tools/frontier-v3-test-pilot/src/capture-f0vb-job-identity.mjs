...
const jobPrefix = values['job-prefix'] ?? 'F0.VB';
if (!/^worker-[0-3]$/.test(values.worker ?? '') || !/^\d+$/.test(values.run ?? '') || !/^[A-Za-z0-9. ]{1,40}$/.test(jobPrefix)
  || !output.startsWith(`${process.cwd()}/`) || !process.env.GITHUB_TOKEN) {
...
for (let attempt = 0; attempt < 20 && !identity; attempt++) {
  ...
  const job = jobs.find((value) => value.name === `${jobPrefix} ${values.worker}` && value.status === 'in_progress'
