...
import { assertPrimaryEvidence, hashJson, mergeSemanticMatrix } from './f02b-native-semantic.mjs';
...
try {
  ...
  const evidence = await Promise.all([...expectedBundles].map(async bundle => {
    const semantic = JSON.parse(await readFile(resolve(root, bundle, 'semantic.json'), 'utf8'));
    const primary = JSON.parse(await readFile(resolve(root, bundle, 'primary.json'), 'utf8'));
    if (hashJson(primary) !== semantic.primarySha256) throw new Error('F0.2B merge rejects primary receipt hash drift');
    assertPrimaryEvidence(primary, semantic);
    return semantic;
  }));
