...
| Harvester creature models and textures | User-provided reference images; OpenAI image generation, project-directed | image-faithful mesh reconstruction begun 2026-08-22 | n/a | Resource-only canonical organic-mesh projects for Biomass Collector, Crusher Stalker and Scythe Stalker. The supplied creature images are the sole likeness authority; 1254×1254 material studies, named animation skeletons and reproducible image-model sources are retained. Cube-only GeckoLib geometry is a legacy compatibility bridge and never an acceptance asset; no entity/runtime registration occurs until an explicit mesh renderer is implemented. |
| Harvester secondary turntables | OpenAI image generation, project-directed from the matching user-supplied base image | generated 2026-08-22 onward | n/a | Retained at `src/main/blender/harvester/references/`. They cannot override the supplied primary image, change primary likeness scoring, or independently justify acceptance. The reviewed Collector v02 manifest is the explicit exception: its named source-local hidden-side camera constraints are canonical only in their own cameras for the shape-only Hunyuan v05 trial; the literal source still controls its trace. Exact prompts and SHA-256 digests are recorded alongside the files. |
