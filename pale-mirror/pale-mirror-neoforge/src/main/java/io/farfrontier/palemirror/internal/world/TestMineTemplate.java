...
public final class TestMineTemplate {
    ...
    public static boolean isMaterialized(ServerLevel level, BlockPos anchor) {
        return level.getBlockState(anchor.offset(-4, 0, -4)).is(Blocks.DEEPSLATE_BRICKS)
                && level.getBlockState(anchor.offset(4, 4, 4)).is(Blocks.DEEPSLATE_BRICKS)
                && level.getBlockState(anchor.above()).is(Blocks.LANTERN);
    }

    public static TestMineRecord observeExisting(ServerLevel level, BlockPos anchor, WorldObjectId id, StoryAudienceId audience) {
        if (!isMaterialized(level, anchor)) throw new IllegalStateException("Test mine marker is absent");
        List<MutableCell> cells = new ArrayList<>();
        NODE_OFFSETS.forEach(offset -> observeCell(level, cells, anchor.offset(offset), InfectionBiomeStage.NODE));
        observeFloorBiomeCells(level, cells, anchor);
        observeApexBiomeCells(level, cells, anchor);
    ...
    private static TestMineRecord placeChecked(ServerLevel level, BlockPos anchor, WorldObjectId id, StoryAudienceId audience) {
        List<MutableCell> cells = new ArrayList<>();
        for (int x = -4; x <= 4; x++) for (int y = 0; y <= 4; y++) for (int z = -4; z <= 4; z++) {
    ...
    private static void observeFloorBiomeCells(ServerLevel level, List<MutableCell> cells, BlockPos anchor) {
        for (int x = -3; x <= 3; x++) for (int z = -3; z <= 3; z++) {
            ...
            InfectionBiomeStage stage = radius <= 1 ? InfectionBiomeStage.FOOTHOLD
                    : radius <= 2 ? InfectionBiomeStage.INFESTED : InfectionBiomeStage.SIEGE;
            observeCell(level, cells, anchor.offset(offset), stage);
        }
    }
    ...
    private static void observeApexBiomeCells(ServerLevel level, List<MutableCell> cells, BlockPos anchor) {
        for (int x = -1; x <= 1; x++) for (int z = -1; z <= 1; z++) observeCell(level, cells, anchor.offset(x, 4, z), InfectionBiomeStage.APEX);
        for (int offset : List.of(-2, 0, 2)) {
            observeCell(level, cells, anchor.offset(-4, 2, offset), InfectionBiomeStage.APEX);
            observeCell(level, cells, anchor.offset(4, 2, offset), InfectionBiomeStage.APEX);
            observeCell(level, cells, anchor.offset(offset, 2, -4), InfectionBiomeStage.APEX);
            observeCell(level, cells, anchor.offset(offset, 2, 4), InfectionBiomeStage.APEX);
        }
    }
    ...
    private static void observeCell(ServerLevel level, List<MutableCell> cells, BlockPos position, InfectionBiomeStage stage) {
        BlockState state = level.getBlockState(position);
        String block = BuiltInRegistries.BLOCK.getKey(state.getBlock()).toString();
        cells.add(new MutableCell(position, block, block, false, stage));
    }
