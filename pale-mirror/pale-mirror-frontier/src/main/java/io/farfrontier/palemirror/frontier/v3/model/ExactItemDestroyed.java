...
public record ExactItemDestroyed(SubjectId itemId, InventoryCustody source, String cause) implements FrontierPayload {
    public ExactItemDestroyed {
        ...
        if (!(source instanceof InventoryCustody.ContainerSlot) && !(source instanceof InventoryCustody.WorldCarrier) && !(source instanceof InventoryCustody.Actor)) {
            throw new IllegalArgumentException("destroyed exact item must have a physical container, actor or world-carrier source");
        }
