...
/** Immutable finance transitions shared by production planning and durable physical completion. */
public final class CompanyWorkPaymentStateSupport {
    private CompanyWorkPaymentStateSupport() { }
    ...
    public static FrontierWorldState settle(FrontierWorldState state, ProductionJob job) {
        return contractFor(state, job).map(contract -> settle(state, job, contract)).orElse(state);
    }
