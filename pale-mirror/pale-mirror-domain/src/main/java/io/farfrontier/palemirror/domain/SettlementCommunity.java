...
public final class SettlementCommunity {
    ...
    public SettlementCommunity(WorldObjectId id) {
        this(id, false, false, CrisisState.NONE, 0);
    }

    public SettlementCommunity(WorldObjectId id, boolean rationing, boolean supplyRequested,
                               CrisisState crisisState, int stableSupplySteps) {
        this.id = Objects.requireNonNull(id, "id");
        if (stableSupplySteps < 0) throw new IllegalArgumentException("Community values must not be negative");
