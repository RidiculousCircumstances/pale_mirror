...
- Manual HOT/COLD evidence is now on the real `DISPLAY=:0`: a live settlement-11 Villager was present in HOT, absent after the 200-tick COLD delay with no forced loading, and a live settlement Villager was present again after the sole audit client rejoined and returned. The temporary respawn gamerule was restored to `false`; PMAudit is now in spectator mode at the audit scene.
- `abf6ae1` adds a bounded immutable-frame audit plan and `/pale_mirror frontier audit list|tp`: the live snapshot exported settlement 5, hive organ 1 and route 5:7, while correctly exporting no nonexistent operation or field post. Player-eye evidence is `screenshots/2026-08-27_08.46.27.png`, `08.47.18.png` and `08.48.04.png`; it proves the views, and exposes label overlap plus a weak route silhouette as honest graybox debt.
- Outer commit `03e501d` makes the active runtime profile idempotent after Packwiz: the old disabled copy is archived, a restored conflicting JAR is disabled, and all seven independent event/content sources plus natural spawns are absent from the verified server startup. Villager Overhaul, PM-sandboxed Crimson/Spore and Mowzie's compatibility carrier remain loaded. One matching audit client is connected, full-screen on real `DISPLAY=:0`, with hash-pinned Freecam and Power Screenshot.
