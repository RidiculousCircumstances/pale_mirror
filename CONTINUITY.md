...
### Now
- Schema v32 separates vanilla-route health from PM write provenance. The canonical route retains its last observed state across chunk unloads; block observations and bounded loaded-chunk reconciliation update a persisted `RailShape` graph, and any connected player reroute between the fixed loading/depot endpoints can restore capacity. Exact baseline/last-applied cells now guard PM writes only. The representative carrier follows the accepted graph path. Core unit/build checks pass; 33/33 GameTests cover a non-authored curved reroute, singleton carrier reconciliation and passive pre-acceptance infection.
