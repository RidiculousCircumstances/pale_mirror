...
- The first executor has a complete pure contract: `CARGO_HANDOFF` can now reconcile immutable exact evidence into hive-store custody. It still needs a NeoForge implementation that materializes the receiver/carrier in naturally loaded chunks, reads actual container contents and submits only that evidence.
- Existing deployed v2 server/runtime state is unchanged and is not evidence for v3.

### Next
- Add the loaded-chunk NeoForge CARGO_HANDOFF executor and postcondition inspector, replacing the temporary restart guard for that supported kind. Then introduce HOT scene leases and execution.
