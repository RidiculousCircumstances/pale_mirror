...
- Wave 2 now has checked recovery boundaries, restartable kernel state, exact bootstrap ownership, persisted sparse spatial/economic state, a true write-ahead canonical commit boundary, an opt-in v3-only lifecycle bridge, bounded infection propagation, exact production, contract-to-cargo flow and one exact COLD cargo route. It still lacks HOT actor execution and physical-intent durability.
- Existing deployed v2 server/runtime state is unchanged and is not evidence for v3.

### Next
- Add physical-intent durability and its recovery/postcondition boundary, then introduce HOT scene leases and execution.
