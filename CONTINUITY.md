...
- Live playtesting found Endermen dismantling the baseline railway by moving its supporting terrain. A server-wide entity-specific griefing guard now disables Enderman pickup and placement while leaving other mobs unchanged; focused GameTest coverage exercises both sides of that policy.
- Live repair testing showed that the route correctly accepted one restored cell but then exposed another unloaded break while Atlas remained generic. Suspended vanilla routes now keep scanning naturally loaded slices, accumulate all known critical breaks, reconcile restored cells in any order, and project the count plus first repair coordinate to debug/Atlas; the representative cart remains absent until canonical capacity returns.
- The disposable private playtest server now runs the checksum-published schema-v31 JAR on its current `world`. Startup reports all PM gameplay adapters `AVAILABLE` except optional FTB Quests; the retained world currently has no bound living region and is ready for a fresh layout-v2 discovery test.
