...
### Now
- Source commit `17a26e7` is deployed in the disposable `frontier-graybox-10-y63-r3-no-structures` server. It is currently running on port 25565 as Java PID 2693059; the world was retained.
...
- The outer-worktree graybox runtime profile is active: it reversibly disables the seven independent event/content sources (Alex's Caves, Born in Chaos, Hostile Tactics, Cataclysm, Naturalist, Ravents and Timeline) and sets all three natural-spawn properties false. Startup evidence proved that Villager Overhaul is a hard Visuals dependency, Crimson/Spore are PM-sandboxed datapack carriers, and Mowzie's is required to deserialize existing entity attachments; these remain loaded without granting simulation authority. One matching audit client is connected on full real `DISPLAY=:0` and has hash-pinned Freecam 1.3.0 and Power Screenshot 1.0.2.
