...
### Now
- The retained disposable `frontier-graybox-10-y63-r3-no-structures` server is running on port 25565 with deployed Pale Mirror JAR SHA-512 `30906430…8cc00f`; exactly one matching PMAudit client is connected at 3840×2400 on real `DISPLAY=:0`. The runtime profile was reapplied after Packwiz refresh, so competing gameplay JARs are again `.graybox-disabled` and natural spawning remains false.
