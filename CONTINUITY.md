...
- The v3 bootstrap now contains three stable functional organs (`HEART`, `BROOD`, `STORE`) at each of its two seed nests. Each `STORE` owns one exact 27-slot hive receiver container. Exact inventory also has a bounded UUID-keyed `WorldCarrier` custody surface with a required reverse index; it is persisted in state schema 8 and rejects dangling or duplicate carrier ownership. This supplies the canonical receiver and carrier identity needed before a real CARGO_HANDOFF can exist. Focused tests and the critical Gradle gate (including 168 GameTests and packaged-JAR verification) pass.
...
- The first executor cannot be attached correctly yet: `CARGO_HANDOFF` records only an observation ID, not immutable physical evidence or the exact post-handoff custody transition. The hive now has canonical organ/container receivers and inventory has exact world-carrier custody; typed evidence plus atomic reconciliation are still required before confirming a dropped item, chest or number.
- Existing deployed v2 server/runtime state is unchanged and is not evidence for v3.

### Next
- Add immutable typed physical observations and make CARGO_HANDOFF reconcile a receiver-container or exact world-carrier result atomically. Then add its NeoForge executor/postcondition inspector, replacing the temporary restart guard for that supported kind; then introduce HOT scene leases and execution.
