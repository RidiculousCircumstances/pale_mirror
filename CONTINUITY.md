...
### Now
- The retained disposable `frontier-graybox-10-y63-r3-no-structures` server is running on port 25565 with the prior Pale Mirror JAR while the verified HOT-absence recovery JAR SHA-512 `ef32abd8…e1ae82` awaits deployment. Exactly one matching PMAudit client is connected at 3840×2400 on real `DISPLAY=:0`.
...
- Outer commit `03e501d` keeps the runtime profile idempotent after Packwiz. The pending profile change disables Alex's Caves, Born in Chaos, Hostile Tactics, Cataclysm, Naturalist, Ravents, RPG Timeline, Crimson Curse and Mowzie's Mobs on both server and audit client, and sets natural animal/monster/NPC spawning false. Blood Moon is not installed; Moonlight is only a retained library; Villager Overhaul is required. Spore remains only because the retained world saves `spore:*` registries, while its native spawning stays disabled and PM owns its sandbox lifecycle.
...
- Deploy and manually prove the HOT-absence recovery in the matching quiet runtime: settlement 5 must show at least one exact Villager after re-admission, with no second client or unrelated gameplay mobs.
