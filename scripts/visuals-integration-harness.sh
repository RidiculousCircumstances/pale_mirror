...
core_jar=${1:?usage: visuals-integration-harness.sh core.jar visuals.jar geckolib.jar villager-overhaul.jar supplementaries.jar moonlight.jar}
...
for input in "$core_jar" "$visuals_jar" "$geckolib_jar" "$villager_jar" \
  "$supplementaries_jar" "$moonlight_jar" "$installer"; do
...
cp "$core_jar" "$visuals_jar" "$geckolib_jar" "$villager_jar" \
  "$supplementaries_jar" "$moonlight_jar" "$runtime_dir/mods/"
