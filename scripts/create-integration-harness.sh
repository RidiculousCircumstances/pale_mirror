...
# A packaged-server companion to the real scheduled-train acceptance walkthrough.
mod_jar=${1:?usage: create-integration-harness.sh /absolute/path/to/pale_mirror.jar /absolute/path/to/create.jar}
...
runtime_dir=$(mktemp -d "${TMPDIR:-/tmp}/pale-mirror-create.XXXXXX")
log_one="$runtime_dir/first-start.log"
log_two="$runtime_dir/restart.log"

fail() { printf 'Create integration harness failed; retained runtime: %s\n' "$runtime_dir" >&2; exit 1; }
for artifact in "$mod_jar" "$create_jar" "$installer"; do [[ -f "$artifact" ]] || fail; done
(
  cd "$runtime_dir"
...
cp "$mod_jar" "$create_jar" "$runtime_dir/mods/"
server_pid=''
...
start_server() {
  ...
  for attempt in $(seq 1 90); do
    if rg -q 'Done \([^)]*\)!' "$log_file"; then
      rg -q 'Pale Mirror bootstrapped' "$log_file" && rg -q 'Create .*initializing' "$log_file" || return 1
