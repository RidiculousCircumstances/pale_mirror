...
public final class FrontierV3DecontaminationGameTests {
    ...
    public static void reagentReducesAndThenClearsOnlyItsOwnedInfectionMarker(GameTestHelper helper) {
        ServerLevel level = helper.getLevel(); BlockPos marker = helper.absolutePos(new BlockPos(30, 8, 0)), chestPosition = marker.east(6);
        ...
        ledger.prepare(cell, markers, InfectionOverlayStage.BLOOM); ledger.activate(cell);
        FrontierV3DecontaminationExecutor.Target reduce = new FrontierV3DecontaminationExecutor.Target(cell, markers, two, container, 0, chestPosition,
        ...
        FrontierV3DecontaminationExecutor.Target clear = new FrontierV3DecontaminationExecutor.Target(cell, markers, one, container, 0, chestPosition,
    ...
    private static List<BlockPos> patch(BlockPos origin) {
        return java.util.stream.IntStream.range(0, FrontierV3InfectionOverlayLedger.PATCH_COLUMNS)
                .mapToObj(index -> origin.offset(index % InfectionCell.BLOCKS, 0, index / InfectionCell.BLOCKS)).toList();
    }
}
